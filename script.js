...
function updatePriorities() {
  ...
  prioritySlides.forEach((slide, index) => {
    ...
    const opacity = Math.max(0, 1 - Math.abs(distance) * 0.72);
    const scale = Math.max(0.86, 1 - Math.abs(distance) * 0.045);
    slide.style.transform = `translate3d(${distance * 100}%,0,0) scale(${scale})`;
...
if (prioritySection) {
  prioritySlides.forEach((slide, index) => {
    slide.style.transform = `translate3d(${index * 100}%,0,0) scale(${index ? 0.955 : 1})`;
