(() => {
  ...
  style.textContent = `
    #program-book-modal{position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;padding:18px;background:rgba(18,18,16,.74);backdrop-filter:blur(5px);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .25s ease,visibility .25s ease}
    ...
    .program-book-close{position:absolute;right:0;top:0;width:44px;height:44px;border:1px solid rgba(255,255,255,.7);border-radius:50%;background:rgba(0,0,0,.25);color:#fff;font-size:26px;line-height:1;cursor:pointer;z-index:30}
    .program-book-stage{position:relative;width:min(920px,92vw);height:min(650px,76vh);perspective:1800px;display:flex;align-items:center;justify-content:center}
    .program-book{position:relative;width:min(860px,88vw);height:min(590px,70vh);transform-style:preserve-3d;filter:drop-shadow(0 24px 30px rgba(0,0,0,.28))}
    ...
    .program-book-page.turn-next{animation:programBookNext .72s cubic-bezier(.22,.61,.36,1) forwards;z-index:12}
    .program-book-page.turn-prev{animation:programBookPrev .72s cubic-bezier(.22,.61,.36,1) forwards;z-index:12}
    @keyframes programBookNext{0%{transform:rotateY(0)}45%{box-shadow:-18px 12px 28px rgba(0,0,0,.20)}100%{transform:rotateY(-180deg)}}
    @keyframes programBookPrev{0%{transform:rotateY(-180deg)}45%{box-shadow:18px 12px 28px rgba(0,0,0,.20)}100%{transform:rotateY(0)}}
  ...
  function turn(direction) {
    if (busy) return;
    const target = current + direction * 2;
    if (target < 0 || target > PROGRAM_PAGES.length - 1) return;
